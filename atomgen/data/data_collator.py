...
class DataCollatorForAtomModeling(DataCollatorMixin):
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if self.pad:
            ...
            if self.return_lap_pe:
                ...
                batch["node_pe"], batch["edge_pe"], batch["attention_mask"] = self.torch_compute_lap_pe(batch["coords"], batch["attention_mask"])
            if self.return_edge_indices:
                # Compute edge indices and distances
                batch["edge_indices"], batch["edge_distances"], batch["attention_mask"] = self.torch_compute_edges(batch["coords"], batch["attention_mask"])
        else:
            ...
            for key in examples[0].keys():
                if key == 'input_ids':
                    ...
                    batch["batch"] =torch.arange(len(examples)).repeat_interleave(torch.tensor(lengths))
                    batch[key] = torch.cat([torch.tensor(sample[key]) for sample in examples], dim=0)
                elif key.startswith('coords') or key.endswith('coords'):
                    batch[key] = torch.cat([torch.tensor(sample[key]) for sample in examples], dim=0)
                elif key.startswith('fixed') or key.endswith('fixed'):
                    batch[key] = torch.cat([torch.tensor(sample[key]) for sample in examples], dim=0)
        ...
        if self.mam:
            ...
            if special_tokens_mask is None:
                special_tokens_mask = [
                    self.tokenizer.get_special_tokens_mask(val, already_has_special_tokens=True) for val in batch["input_ids"].tolist()
                ]
    ...
    def torch_mask_tokens(self, inputs: Any, t: Any, special_tokens_mask: Optional[Any] = None) -> Tuple[Any, Any, Any]:
        """
        ...
        inputs = torch.where(mask, inputs, self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token))
    ...
    def torch_compute_edges(self, coords: Any, attention_mask: Any) -> Any:
        ...
        _, topk_indices = torch.topk(dist_matrix, k=min(self.max_neighbors, dist_matrix.size(2)), dim=2, largest=False)
        ...
        edge_indices = pad_sequence(edge_indices_list, batch_first=True, padding_value=0)
        edge_distances = pad_sequence(distance_list, batch_first=True, padding_value=-1)
        edge_attention_mask = torch.cat([torch.cat([torch.ones(1, l), torch.zeros(1, edge_indices.size(1)-l)], dim=1) for l in lengths], dim=0)
    ...
    def torch_compute_lap_pe(self, coords: Any, attention_mask: Any) -> Any:
        ...
        laplacian_matrix = torch.eye(N) - degree_matrix @ adjacency_matrix @ degree_matrix
        ...
        for i, edge_indices in enumerate(edge_indices_list):
            ...
            all_edges_pe[:, :self.k] = torch.index_select(node_pe[i], 0, edge_indices[:, 0])
            all_edges_pe[:, self.k:] = torch.index_select(node_pe[i], 0, edge_indices[:, 1])
        ...
        edge_attention_mask = torch.cat([torch.cat([torch.ones(1, l), torch.zeros(1, edge_pe.size(1)-l)], dim=1) for l in lengths], dim=0)
        attention_mask = torch.cat([attention_mask, edge_attention_mask], dim=1)
