    ...
    with h5py.File(file_path, "r") as f:
        ...
        for key in tqdm(keys, desc="Processing SPICE dataset"):
            ...
            for i in range(num_conformations):
                dataset['input_ids'].append(f[key]['atomic_numbers'][()].astype('int16'))
                dataset['coords'].append(0.529177249 * f[key]['conformations'][()][i])
                dataset['forces'].append(-27.211407953 * f[key]['dft_total_gradient'][()][i])
                dataset['formation_energy'].append(27.211407953 * f[key]['formation_energy'][()][i])
                dataset['total_energy'].append(27.211407953 * f[key]['dft_total_energy'][()][i])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Preprocess SPICE dataset into a HuggingFace dataset.')
